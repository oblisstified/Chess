...
def RandomMove(movecount):
    ...
    blackarray=ValidMove(movecount,xco,yco,Strpiece)
    ...
    for Row in range(8):
        for Column in range(8):
            ...
            if StrGrid[Column][Row][0]=='b':
                array=ValidMove(movecount,Row,Column,StrGrid[Column][Row])
                print('hello')
                if len(array)!=0:
                    print('ok')
                    for i in array:
...
while Endgame:
    ...
    if movecount%2==1:
        ...
        if CalcHighestPiece(movecount) == True :
            print('ok')
        else:
