...
def Check(movecount,row,column,Strpiece):
    bkingpos=bKingPosition()
    wkingpos=wKingPosition() 
    
    if Strpiece[0]=='w':
         
        krow=int(bkingpos[0])
        kcolumn=int(bkingpos[2])
        OneSqaure(movecount,krow,kcolumn)
        screen.blit(bking,(5+krow*60,5+kcolumn*60))
        if bkingpos in ValidMove(movecount,row,column,Strpiece):

            pygame.draw.rect(screen,red,(5+60*krow,5+60*kcolumn,60,60))
            screen.blit(bking,(5+krow*60,5+kcolumn*60))
            pygame.display.update()
            return True
        ...
        krow=int(wkingpos[0])
        kcolumn=int(wkingpos[2])
        OneSqaure(movecount,krow,kcolumn)
        screen.blit(wking,(5+krow*60,5+kcolumn*60))
        if wkingpos in ValidMove(movecount,row,column,Strpiece):
    ...
    for Row in range(8):
        for Column in range(8):
            if StrGrid[Column][Row]=='wbishop':
                print(ValidMove(movecount,Row,Column,StrGrid[Column][Row]))
    return False

        
    
def CheckBoard(movecount):
    bkingpos=bKingPosition()
    krow=int(bkingpos[0])
    kcolumn=int(bkingpos[2])
    if movecount%2==0:
        ...
        for Row in range(8):
            for Column in range(8):
                ...
                if Strpiece[0]=='w':
                    print(ValidMove(movecount,Row,Column,Strpiece),Strpiece)
                    if bkingpos in ValidMove(movecount,Row,Column,Strpiece):
                        print('ok')
                        pygame.draw.rect(screen,red,(5+60*krow,5+60*kcolumn,60,60))
                        screen.blit(wking,(5+krow*60,5+kcolumn*60))
        ...
        for p in range(-1,2):
            if 0<=(Column+p)<8 and 0<=Row+i<8:
                if i==0 and p==0:
                    ...
                    move=(str(Row+i)+'+'+str(Column+p))
                    if move in ValidMove(movecount,Row,Column,'wking'):
        ...
        if event.type ==pygame.MOUSEBUTTONUP and event.button == 1:
            for Row2 in range(8):
                for Column2 in range(8):
                    if 5+Row2*60<mouse[0]<65+Row2*60 and 5+Column2*60<mouse[1]<65+Column2*60:
                        var=str(Row2)+'+'+str(Column2)
                        
                        if var in ValidMove(movecount,row,column,Strpiece):
                            if Strpiece[0]==wob:
                                number1=Row2+Column2
                                if number1%2==0:
                                    colour1=white
                                else:
                                    colour1 = grey
                                ...
                                pygame.draw.rect(screen,colour1,(5+Row2*60,5+Column2*60,60,60))


                                AfterClickCircle(movecount,row,column,piece)
                                pygame.draw.rect(screen,colour1,(5+Row2*60,5+Column2*60,60,60))
                                screen.blit(piece,(5+Row2*60,5+Column2*60)) 

                                pygame.display.update()
                                Check(movecount,Row2,Column2,Strpiece)
                                for Row in range(8):
                                    for Column in range(8):
                                        if (StrGrid[Column][Row])[0]=='w':
                                            print(ValidMove(movecount,Row,Column,StrGrid[Column][Row]),StrGrid[Column][Row])
                                Grid[Column2][Row2]=piece
                                StrGrid[Column2][Row2]=Strpiece
